...
app.post('/api/patients', authenticateToken, (req, res) => {
    ...
    const query = 'INSERT INTO Paciente (Nombre, Apellido, FechaNacimiento, Direccion, Telefono, Email) VALUES (?, ?, ?, ?, ?, ?)';

    db.query(query, [nombre, apellido, fechaNacimiento, direccion, telefono, email], (err, results) => {
        if (err) return res.status(500).json({ error: 'Database error' });
...
app.put('/api/patients/:id', authenticateToken, (req, res) => {
    const { nombre, apellido, fechaNacimiento, direccion, telefono, email } = req.body;
    const query = 'UPDATE Paciente SET Nombre = ?, Apellido = ?, FechaNacimiento = ?, Direccion = ?, Telefono = ?, Email = ? WHERE ID = ?';

    db.query(query, [nombre, apellido, fechaNacimiento, direccion, telefono, email, req.params.id], (err, results) => {
        if (err) return res.status(500).json({ error: 'Database error' });
...
app.post('/api/evaluaciones', async (req, res) => {
    try {
        ...
        if (!PacienteID || !EspecialistaID || !TipoEvaluacion || !Fecha) {
            console.error('Faltan campos requeridos:', { PacienteID, EspecialistaID, TipoEvaluacion, Fecha });
      ...
      `;
        ...
        db.query(query, [PacienteID, EspecialistaID, Fecha, TipoEvaluacion], (err, results) => {
            if (err) {
                console.error('Error en la consulta SQL:', err);
                console.error('Query:', query);
                console.error('Parámetros:', [PacienteID, EspecialistaID, Fecha, TipoEvaluacion]);
...
app.post('/api/ados', (req, res) => {
    try {
        const { EvaluacionID, Actividad, Observacion, Puntuacion, Modulo, CategoriaID } = req.body;
      ...
      `;
        ...
        db.query(query, [EvaluacionID, Actividad, Observacion, Puntuacion, Modulo, CategoriaID], (err, results) => {
            if (err) {
...
app.post('/api/reportes/', authenticateToken, (req, res) => {
    try {
      ...
      `;
        ...
        db.query(query, [EvaluacionID, FechaGeneracion, Contenido], (err, results) => {
            if (err) {
...
app.put('/api/especialista/:id', authenticateToken, (req, res) => {
    ...
    const query = 'UPDATE Especialista SET Nombre = ?, Apellido = ?, Email = ? WHERE ID = ?';
...
app.put('/api/administrador/:id', authenticateToken, (req, res) => {
    const { nombre, apellido, email } = req.body;
    const query = 'UPDATE Administrador SET Nombre = ?, Apellido = ?, Email = ? WHERE ID = ?';
...
app.post('/api/especialista', authenticateToken, (req, res) => {
    ...
    const query = 'INSERT INTO Especialista (Nombre, Apellido, Email, Contrasena) VALUES (?, ?, ?, ?)';
